...
void enfiler(File *file, int nv_mouvement)
{
    mouvement *nouveau = malloc(sizeof(*nouveau));
    if (file == NULL || nouveau == NULL)
    {
        exit(EXIT_FAILURE);
    }

    nouveau->mv_mouvement = nv_mouvement;
    nouveau->suivant = NULL;

    if (file->premier != NULL) /* La file n'est pas vide */
    {
        /* On se positionne à la fin de la file */
        mouvement *mouvement_Actuel = file->premier;
        while (mouvement_Actuel->suivant != NULL)
        {
            mouvement_Actuel = mouvement_Actuel->suivant;
        }
        mouvement_Actuel->suivant = nouveau;
    }
    else /* La file est vide, notre mouvement est le premier */
    {
...
int historique(File *file)    //defiler la File que nous avons remplie précedemment par les 10 derniers coups
{
    if (file == NULL)
    {
        exit(EXIT_FAILURE);
    ...
    int mouvementDefile = 0;

    /* On vérifie s'il y a quelque chose à défiler */
    if (file->premier != NULL)
    {
        mouvement *mouvementDefile = file->premier;

        mouvementDefile = mouvementDefile->nv_mouvement;
        file->premier = mouvementDefile->suivant;
        free(mouvementDefile);
    }

    return mouvementDefile;
}  
  ...
  do
  {
    ...
    switch(choix){
        ...
        case't':
              ...
              if (texte==mdp[10])
              {
                  printf("voici vos top 10 meilleurs scores \n");
                  Top10_joueur()
              }
              else
                printf("voici les top 10 meilleurs scores du jeu\n");
                Top10_jeu()
        break;
